...
p_load(caret, dplyr, randomForest, rpart, rpart, rattle)
...
# Model Fitting
system.time(
  DT_model <- caret::train(Outcome ~., data = training[1:5000,], 
                           method = "rpart", tuneLength = 5)
)
plot(DT_model) # Accuracy as a function of complexity parameter (cp)

DT_model$bestTune # best cp value
...
# Prediction on Test Data 
test_pred_DT <- predict(DT_model, newdata = testing)
# Newdata is the data on which we want to make prediction. 
# You can supply any data you want

# Predicting probabilities instead of class 
test_pred_DT_prob <- predict(DT_model, newdata = testing,
                                 type = "prob")

saveRDS(DT_model,"DT_model.RDS")
# Confusion Matrix
CF <- caret::confusionMatrix(test_pred_DT, testing$Outcome, positive = "1")

CF$table
CF$overall[1]
DTAccuracy <- CF$overall[1]
